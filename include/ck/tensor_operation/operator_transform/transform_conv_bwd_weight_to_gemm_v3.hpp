...
struct TransformConvBwdWeightToGemmV3
{
    ...
    {
        const index_t NStride = output_strides[1];
        const index_t WoStride = output_strides[4];
        const auto KStride     = output_strides[2];
        const auto desc=  make_naive_tensor_descriptor(make_tuple( N, K, Ho * Wo),
                                            make_tuple(NStride, KStride, WoStride));
                      ...
                      const std::array<index_t, NDimSpatial + 3>& input_strides)
    {
        ...
        // else
        // {
            return make_naive_tensor_descriptor(make_tuple(N, C, Hi, Wi),
                       ...
                       const std::array<index_t, NDimSpatial + 3>& weights_strides)
    {
        ...
        return make_naive_tensor_descriptor(make_tuple(K, C * Y * X), make_tuple(KStride, XStride));
    }
        ...
        // {
            ...
            const auto out_gemmkbatch_gemmk0_gemmm_gemmk1_pad_grid_desc =
                transform_tensor_descriptor(
                    out_gemmkbatch_gemmk0_gemmm_gemmk1_grid_desc,
                    make_tuple(
                               make_pass_through_transform(GemmKBatch * GemmK0),
                               make_right_pad_transform(GemmM, PadGemmM),
                               make_pass_through_transform(GemmK1Number)),
                    make_tuple(Sequence<0>{}, Sequence<1>{}, Sequence<2>{}),
                    make_tuple(Sequence<0>{}, Sequence<1>{}, Sequence<2>{}));
