...
auto create_args(int argc, char* argv[])
{
    ...
    arg_parser.insert("v", "1", "0:no validation, 2:cpu validation, 2:gpu validation(experimental)")
        ...
        .insert("d", "576", "head dim for q, k")
        .insert("d_v", "512", "head dim for v, -1 means equal to d")
        .insert("scale_s",
                "0",
...
bool run(const ck_tile::ArgParser& arg_parser)
{
    ...
    const auto init_args = [&, k_paddings_ = seqlen_kpads](auto& args) {
        ...
        const ck_tile::index_t stride_v    = [&]() {
            if(is_v_rowmajor)
                return i_perm ? hdim_q : nhead_k * hdim_q;
            else
        ...
        const ck_tile::index_t nhead_stride_v    = [&]() {
            if(is_v_rowmajor)
                return 0 < page_block_size ? (i_perm ? page_block_size * hdim_q : hdim_q)
                                              : (i_perm ? shape_seqlen_k * hdim_q : hdim_q);
            else
                return 0 < page_block_size ? (i_perm ? hdim_q * page_block_size : page_block_size)
                                              : (i_perm ? hdim_q * shape_seqlen_k : shape_seqlen_k);
        }();
        const ck_tile::index_t nhead_stride_vnew = [&]() {
            if(is_v_rowmajor)
                return i_perm ? seqlen_knew * hdim_q : hdim_q;
            else
                return i_perm ? hdim_q * seqlen_knew : seqlen_knew;
        }();
        ...
        const ck_tile::index_t batch_stride_v =
            (0 < page_block_size ? (nhead_k * hdim_q * page_block_size)
                                 : (nhead_k * hdim_q * shape_seqlen_k));
        ...
        args.v_ptr = k_buf.GetDeviceBuffer();
    ...
    if(do_validation == 2)
    {
        ...
        naive_a.v_ptr           = k_buf.GetDeviceBuffer();
        ...
        naive_attention_fwd<true>(naive_t, naive_a, naive_s);
